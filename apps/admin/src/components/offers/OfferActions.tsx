...
export function OfferActions({ offerId, status }: Props) {
  ...
  async function callAction(action: "activate" | "pause" | "archive") {
    ...
    try {
      ...
      toast.success(OFFER_MSGS[action]);
      router.refresh();
    } catch (err) {
      toast.error(err instanceof Error ? err.message : `Could not ${action} offer. Please try again.`);
    } finally {
      setLoading(null);
    }
  }
