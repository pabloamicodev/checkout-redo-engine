...
export const metadata = { title: "Checkout Test — MarginLab" };

const analyticsService = new AnalyticsService();

export default async function CheckoutTestDetailPage({
  params,
  searchParams,
}: {
  ...
  const experiment = await prisma.experiment.findFirst({
    where: { id, shopId: shop.id, type: "CHECKOUT_TEST" },
    select: {
  ...
  return (
    <ExperimentDetailShell
      ...
      tab={tab}
      breadcrumb={{ href: "/checkout-tests", label: "Checkout Tests" }}
    />
  );
}
