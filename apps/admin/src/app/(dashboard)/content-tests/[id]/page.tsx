...
export const metadata = { title: "Content Test — MarginLab" };

const analyticsService = new AnalyticsService();

export default async function ContentTestDetailPage({
  params,
  searchParams,
}: {
  ...
  const experiment = await prisma.experiment.findFirst({
    where: { id, shopId: shop.id, type: "CONTENT_TEST" },
    select: {
  ...
  return (
    <ExperimentDetailShell
      ...
      tab={tab}
      breadcrumb={{ href: "/content-tests", label: "Content Tests" }}
    />
  );
}
