...
export const metadata = { title: "Offer Test — MarginLab" };

const analyticsService = new AnalyticsService();

export default async function OfferTestDetailPage({
  params,
  searchParams,
}: {
  ...
  const experiment = await prisma.experiment.findFirst({
    where: { id, shopId: shop.id, type: "OFFER_TEST" },
    select: {
  ...
  return (
    <ExperimentDetailShell
      ...
      tab={tab}
      breadcrumb={{ href: "/offer-tests", label: "Offer Tests" }}
    />
  );
}
